...
export const createVideo = async (req, res) => {
  if (req.files == null) return res.json({ error: 'ویدیو انتخاب نشد' });
  ...
  if (!allowedType.includes(ext.toLowerCase())) {
    return res.json({ error: ' .mp4 * فرمت ویدیو معتبر نیست و از مجاز ' });
  }
  if (fileSize > 5000000)
    return res.json({ error: '.حجم ویدیو نباید 5 مگابایت بیشتر باشد' });
